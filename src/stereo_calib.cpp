...
void stereo_calib(const cv::Mat& img_L, const cv::Mat& img_R, Mat& R, Mat& T, Mat& K_L,Mat& K_R,Mat& D1,Mat& D2) {
  ...
  if (foundL == true && foundR == true) {
    ...
    std::cout << "Points Not founded,findChessboardCorners failed!" << std::endl;
  }
  
  for (int i = 0; i < board_Row; i++) {
    for (int j = 0; j < board_Col; j++) {
      objectpoint.push_back(cv::Point3f(i * squaresize, j * squaresize, 0.0f));//squarsize:30.3mm
    }
  }
